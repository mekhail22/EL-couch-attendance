"""
تطبيق إدارة أكاديمية كرة القدم "الكوتش أكاديمي"
- نظام حضور وغياب
- إدارة اشتراكات ومدفوعات
- لوحة تحكم للكابتن ولوحة خاصة لكل لاعب
- استخدام Google Sheets كقاعدة بيانات
- دعم جلسات الدخول (Session)
...
def get_google_sheets_client():
    ...
    """
    try:
        # قراءة بيانات خدمة الحساب من secrets
...
def save_dataframe(sheet_name: str, df: pd.DataFrame):
    ...
    if not df.empty:
        ...
        if sheet_name == "Users":
            worksheet.update([["username", "password", "role"]])
        elif sheet_name == "Attendance":
            worksheet.update([["player_name", "date", "status", "recorded_by"]])
        elif sheet_name == "Subscriptions":
            worksheet.update([["player_name", "monthly_fee", "start_date", "end_date", "subscription_status"]])
        elif sheet_name == "Payments":
            worksheet.update([["player_name", "amount", "payment_method", "payment_date", "notes"]])
...
def is_valid_three_part_name(name: str) -> bool:
    ...
    يسمح بوجود مسافات إضافية في البداية والنهاية
    """
    if not name or not isinstance(name, str):
        return False
    parts = name.strip().split()
    # الاسم ثلاثي: يجب أن يحتوي على 3 أجزاء على الأقل (يمكن قبول أكثر؟ حسب المطلب: ثلاثي فقط)
...
def hash_password(password: str) -> str:
    """
    تشفير كلمة المرور (اختياري - يمكن استخدامه لزيادة الأمان)
    """
...
def add_new_user(username: str, password: str, role: str) -> bool:
    """
...
# ============================================
# لوحة تحكم الكابتن (المدرب)
# ============================================

def coach_dashboard():
    """
    عرض لوحة تحكم الكابتن بجميع التبويبات
    """
    display_header("لوحة تحكم الكابتن", "إدارة الحضور، الاشتراكات، والمدفوعات")
    
    # عرض معلومات إضافية في الشريط الجانبي
